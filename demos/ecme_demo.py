...
from icdm2025.methods.ecme import ECME_algorithm
from icdm2025.core.fit_on_source import fitdag  # reuse our GLS DAG fit

def build_parent_list(amat: np.ndarray) -> list[list[int]]:
    p = amat.shape[0]
    return [[j for j in range(p) if amat[j, i] == 1 and j != i] for i in range(p)]

def main(data_dir: str = "data", tol: float = 1e-6, max_iter: int = 3000):
    # --- Load data ---
    ...
    # --- Run ECME ---
    dag_fit, Sigma_hat, iters, converged = ECME_algorithm(
        X_obs=X_obs,
        idx_t=idx_t,
        amat=amat,
    ...
    print(f"ECME: conv={converged}, iters={iters}, MAE={mae:.4f}, RMSE={rmse:.4f}, R^2={r2:.4f}")
    ...
    plt.tight_layout()
    out = os.path.join("outputs", "ECME_Cov.pdf")
